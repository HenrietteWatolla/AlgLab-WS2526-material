...
class Heuristics:
    ...
    def multi_start_greedy(graph: nx.Graph, runs: int = 50, seed: int | None = None) -> Coloring:
        ...
        for i in range(runs):
            ...
            coloring = Heuristics.greedy_coloring(graph, order)
            k = Heuristics.num_colors(coloring)
    ...
    def dsatur_coloring(graph: nx.Graph) -> dict[int, int]:
        ...
        while uncolored:
            ...
            v = max(
                uncolored,
                key = lambda x: (
                    Heuristics.saturation_degree(graph, x, coloring),
                    degree[x]
                )
            )
